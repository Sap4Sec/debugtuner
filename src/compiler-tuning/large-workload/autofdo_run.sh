...
if [ $PROFILES = true ] && [ $EXISTING = false ]; then
        ...
        # Build llvm with baseline-$VERSION LLVM
        ...
        rm -rf $PATH_TO_BENCHMARK_LLVM_BUILD
        mkdir -p $PATH_TO_BENCHMARK_LLVM_BUILD && cd $PATH_TO_BENCHMARK_LLVM_BUILD
        mkdir -p symlinks && cd symlinks
        ln -sf $PATH_TO_LLVM_BUILDS/$NAME/bin/clang clang
        ln -sf $PATH_TO_LLVM_BUILDS/$NAME/bin/clang clang++

        cd $PATH_TO_BENCHMARK_LLVM_BUILD
        ...
        mkdir -p $PATH_TO_PROFILES/$NAME
        cp perf.data $PATH_TO_PROFILES/$NAME/.

        mkdir -p $PATH_TO_LOGS
        cd $PATH_TO_PROFILES/$NAME
        $PATH_TO_CREATE_LLVM_PROF/create_llvm_prof --binary $PATH_TO_BINARIES/$NAME/clang --out clang.prof &> $PATH_TO_LOGS/prof_$NAME.log

        rm -rf $PATH_TO_AUTOFDO_LLVM_BUILDS/$NAME
        mkdir -p $PATH_TO_AUTOFDO_LLVM_BUILDS/$NAME && cd $PATH_TO_AUTOFDO_LLVM_BUILDS/$NAME
        cmake -G Ninja "${COMMON_BENCH_CMAKE_FLAGS[@]}" \
            "${CMAKE_COMPILER[@]}" \
            -DCMAKE_C_FLAGS="-fprofile-sample-use=${PATH_TO_PROFILES}/$NAME/clang.prof" \
            -DCMAKE_CXX_FLAGS="-fprofile-sample-use=${PATH_TO_PROFILES}/$NAME/clang.prof" \
            ${PATH_TO_SOURCES}/llvm-project/llvm
        ninja -j 20 clang

        mkdir -p $PATH_TO_BINARIES/autofdo/$NAME
        cp bin/clang $PATH_TO_BINARIES/autofdo/$NAME/.
...
ln -sf $PATH_TO_AUTOFDO_LLVM_BUILDS/$NAME/bin/clang clang
ln -sf $PATH_TO_AUTOFDO_LLVM_BUILDS/$NAME/bin/clang clang++
...
# --- Run hyperfine script ---
mkdir -p $PATH_TO_RESULTS
hyperfine --runs 10 "./hyperfine_run_core_0.sh" \
    --show-output \
